...
```bash
# Poll for progress
curl -X GET "https://your-domain.com/api/watermark/progress/9171cdcb49e640c5b2d6d22d04456f7a?filename=audio.wav" \
...
async function checkProgress(taskId, filename) {
  const response = await fetch(
    `https://your-domain.com/api/watermark/progress/${taskId}?filename=${encodeURIComponent(filename)}`,
    {
      headers: {
        'Authorization': 'Bearer origid_xxxxxxxxxxxxxxxx',
      },
...
def poll_until_complete(task_id, filename, api_key):
    ...
    url = f'https://your-domain.com/api/watermark/progress/{task_id}'
