...
```bash
curl -X GET "https://your-domain.com/api/watermark/download/9171cdcb49e640c5b2d6d22d04456f7a?filename=audio.wav" \
...
async function downloadWatermarkedFile(taskId, filename) {
  const response = await fetch(
    `https://your-domain.com/api/watermark/download/${taskId}?filename=${encodeURIComponent(filename)}`,
    {
      headers: {
        'Authorization': 'Bearer origid_xxxxxxxxxxxxxxxx',
      },
...
def download_watermarked_file(task_id, filename, output_path, api_key):
    ...
    url = f'https://your-domain.com/api/watermark/download/{task_id}'
